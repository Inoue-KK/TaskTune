...
- Todoの追加（シートUI）。期日（日時）を設定可能なトグル付き。期日設定時に繰り返し間隔（Daily/Weekly/Monthly）も選択可能
...
- `Todo` モデルは `dueDate: Date?`・`notificationID: String`・`repeatInterval: RepeatInterval?`・`missedCount: Int` を持つ。SwiftData は optional / デフォルト値付きプロパティのライトウェイトマイグレーションを自動処理する
- `RepeatInterval` enum（`Todo.swift` に定義）は `String, Codable, CaseIterable`。`calendarComponent` プロパティで Calendar.Component に変換
- 通知は `NotificationManager`（`@MainActor` シングルトン）が一元管理。`UNUserNotificationCenterDelegate` を実装してフォアグラウンド時もバナー表示。通知の `userInfo` に `notificationID`・`listTitle` を埋め込み。「Mark Complete」アクション（カテゴリ `TODO_DUE`）で通知から直接完了
- 繰り返しTodoの自動前進ロジックは `todo_listApp.swift` の `advanceOverdueRepeatingTodos(in:)` 関数で実装。`container.mainContext` を使用して SwiftUI の `@Query` と同一コンテキストで操作
- `AddTodoView` のシート高さは期日トグルの状態に応じて `ContentView` 側で動的に切り替え（340pt / 440pt）
- `EditTodoView` のシート高さは内部の `selectedDetent` で動的に切り替え（340pt / 440pt）
