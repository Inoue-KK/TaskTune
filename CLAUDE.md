...
- アクセントカラー（`accentColor`）は hex 文字列で保存。`Color(hex:)` / `.hexString` 拡張（`SettingsView.swift`）で変換。+ ボタン（`.glassEffect` tint）・歯車アイコン・Todoチェックマーク・トグルスイッチに適用。テキストへのグローバル tint は使わない（淡い色でも視認性を確保するため）
