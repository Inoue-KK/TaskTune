...
- Todoの追加（シートUI）
- タップでチェックON/OFF（アニメーション付き）。完了時にサウンド・ハプティクスを発火（設定でON/OFF可能）
...
- サウンドは `AudioServicesPlaySystemSound(1054)`（`AudioToolbox`）、ハプティクスは `.sensoryFeedback(.success)` を使用。どちらも Todo 完了時のみ発火
- 設定値は `@AppStorage`（`soundEnabled`・`hapticEnabled`）で永続化
